...
class MaskedRegionProcessor(BaseProcessor):
    ...
    def __call__(self, item, mask=True, num_boxes=99999):
        image_labels = []

        for i in range(item.shape[0]):
            if mask and i <num_boxes:
                prob = random.random()
...
@registry.register_processor("electra_masked_region")
class ElectraMaskedRegionProcessor(BaseProcessor):
    """
    Masks a region with probability `mask_probability`
    """
