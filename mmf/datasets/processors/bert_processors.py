...
class MaskedTokenProcessor(BaseProcessor):
    ...
    def __call__(self, item, prob=None):
        text_a = item["text_a"]
        ...
        output = self._convert_to_indices(
            tokens_a, tokens_b, probability=self._probability if prob is None else prob
        )
...
@registry.register_processor("electra_masked_token")
class ElectraMaskedTokenProcessor(MaskedTokenProcessor):
    _CLS_TOKEN = "[CLS]"
    _SEP_TOKEN = "[SEP]"
