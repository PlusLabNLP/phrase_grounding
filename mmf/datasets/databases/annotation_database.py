...
class AnnotationDatabase(torch.utils.data.Dataset):
    ...
    def load_annotation_db(self, path):
        ...
        elif path.find("flickr30k/all") != -1:
            self._load_flickr30k_retrieval(path)
        elif path.find("flickr30k") != -1:
            self._load_flickr30k(path)
        elif path.find("cocoRet") != -1:
            self._load_coco_retrieval(path)
    ...
    def _load_coco_retrieval(self, path):
        from mmf.datasets.builders.coco_retrieval.database \
            import COCORetrievalDatabase

        test_id_file_path = self.config.get('test_id_file_path', None)
        hard_neg_file_path = self.config.get('hard_neg_file_path', None)

        self.data = COCORetrievalDatabase(path,
                                               self.dataset_type,
                                               test_id_file_path,
                                               hard_neg_file_path
                                               )
