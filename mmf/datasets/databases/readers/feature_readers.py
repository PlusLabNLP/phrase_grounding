...
class LMDBFeatureReader(PaddedFasterRCNNFeatureReader):
    ...
    def _load(self, image_file_path):
        if self.env is None or not hasattr(self, 'image_id_indices'):
            self._init_db()
        ...
        try:
            ...
            # default
            #print(self.image_id_indices.keys())
            ...
            try:
                ...
                try:
                    image_id = str(split)#.encode('UTF-8')
                    img_id_idx = self.image_id_indices[image_id]
                except:
                    image_id = ('v7w_'+str(split)).encode() 
                    '''print(list(self.image_id_indices.keys())[:10])
                    print(len(list(self.image_id_indices.keys())))
                    o = open('temp.txt', 'w')
                    for l in list(self.image_id_indices.keys()):
                        o.write(str(l)+'\n')'''
