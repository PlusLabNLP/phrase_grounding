...
class COCORetrievalDataset(MMFDataset):
    def __init__(self, config, *args, dataset_name="coco_retrieval", **kwargs):
    ...
    def __getitem__(self, idx):
        ...
        if len(sample_info) == 2:
            ...
            for cnt, image_id in enumerate(sample_info[1]):
                features1, mix_num_boxes1, spatials1, image_mask1 = \
                    self.get_feature(image_id)
                features.append(features1)
                spatials.append(spatials1)
                image_mask.append(image_mask1)
                caption.append(caption1)
                input_mask.append(input_mask1)
                segment_ids.append(segment_ids1)
                if image_id == sample_info[0]['image_id']:
                    target = torch.tensor(cnt).long()
                
                
            
            features = torch.stack(features, dim=0)
            spatials = torch.stack(spatials, dim=0)
            image_mask = torch.stack(
                image_mask, dim=0
            )
            caption = torch.stack(caption, dim=0)
    ...
    def get_feature(self, image_id):
        ...
        boxes = features_data['image_info_0']['boxes']
