...
from mmf.modules.embeddings import BertVisioLinguisticEmbeddings, ImageBertVisioLinguisticEmbeddings, TempImageBertVisioLinguisticEmbeddings
...
class VisualBERTBase(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoded_layers = self.encoder(
            embedding_output, extended_attention_mask, self.fixed_head_masks, output_hidden_states=output_hidden_states,
        )
        if output_hidden_states:
            sequence_output = encoded_layers[1][-6]
        else:
...
class VisualBERTForPretraining(nn.Module):
    ...
    def forward(
        ...
        image_embeddings=(torch.matmul(image_text_label, self.bert.embeddings.word_embeddings.weight)) if image_text_label is not None else None
        #image_embeddings = None
...
class VisualBERTForClassification(nn.Module):
    def __init__(self, config):
        ...
        if self.config.training_head_type == 'flickr30k' or self.config.training_head_type == 'refcoco' or self.config.training_head_type == 'visual7w' or self.config.training_head_type == 'flickr30k_retrieval' or self.config.training_head_type == 'coco_retrieval':
    ...
    def forward(
        ...
        sequence_output, pooled_output, attention_weights = self.bert(
            ...
            visual_embeddings_type,
            image_text_alignment,
            output_hidden_states=('flickr30k' in self.config.training_head_type or 'refcoco' in self.config.training_head_type),
            image_embeddings=image_embeddings,
            image_info=image_info,
        )

        if 'flickr30k_retrieval' in self.config.training_head_type:
            ...
            avg_img = []
            sid = 1
            eid = (input_ids[0] == 102).nonzero()
            seq_t = sequence_output_t[:, sid:eid, :]
            seq_t = seq_t/torch.norm(seq_t, 2, dim=-1, keepdim=True)
            sequence_output_v = sequence_output_v/torch.norm(sequence_output_v, 2, dim=-1, keepdim=True)

            att = torch.matmul(seq_t, sequence_output_v.transpose(-1, -2)) #/math.sqrt(768) #2, 1
            bias = (1-image_attention_mask.unsqueeze(1)) * -1000000.0
            att1 = nn.Softmax(dim=-1)(att+bias)#torch.matmul(seq_t, sequence_output_v.transpose(-1, -2)) /math.sqrt(768) #2, 1
            att2 = nn.Softmax(dim=-2)(att+bias)#torch.matmul(seq_t, sequence_output_v.transpose(-1, -2)) /math.sqrt(768) #2, 1
            mix_att = att1*att2
            return torch.sum(torch.sum(mix_att, -1), -1) / torch.sum(image_attention_mask, -1)
        ...
        if self.training_head_type == 'visual7w':
            ...
            logits = logits[:, 101:]
            logits = logits.squeeze(2).gather(1, multiple_choice_idx) 
            act = nn.Sigmoid()
            logits = act(logits)
            output_dict["scores"] = logits #logits[:, -self.config.num_multiple_choices:].squeeze()
            ...
            for i, (sid, eid) in enumerate(zip(start_ind, end_ind)):
                ...
                for _ in range(0):
                    att = torch.matmul(seq_t2, sequence_output_v[i].transpose(-1, -2)) /math.sqrt(768) #1or2, 64
                    att = nn.Softmax(dim=-1)(att)
                    seq_v = torch.matmul(att, sequence_output_v[i]) #1, 768
                    att = torch.matmul(seq_t, seq_v.transpose(-1, -2)) /math.sqrt(768) #2, 1
                    att = nn.Softmax(dim=0)(att)
                    seq_t2=(torch.sum(att*seq_t, 0)).unsqueeze(0)
                avg_text.append(seq_t2.squeeze(0))
                
                
            avg_text = torch.stack(avg_text)
            sim = torch.matmul(sequence_output_v, avg_text.unsqueeze(-1))
            output_dict["scores"] = 100*sim
            return output_dict #'''
            ...
            if start_ind is not None:
                ...
                avg_text = torch.stack(avg_text)
                sim = torch.matmul(sequence_output_v, avg_text.unsqueeze(-1))
                mask_tgt = (1-cls_prob_mask.float()) * -1000000.
                try:
                    sim = (sim + mask_tgt.unsqueeze(-1))
                except:
                    sim = (sim + mask_tgt)#'''
                output_dict["scores"] = 100*sim 
                return output_dict #'''
            if start_ind is not None:
                avg_text = []
                for i, (sid, eid) in enumerate(zip(start_ind, end_ind)):
                    avg_text.append(torch.mean(sequence_output_t[i, sid+1:eid+1, :], 0))
                avg_text = torch.stack(avg_text)
                sim = torch.matmul(sequence_output_v, avg_text.unsqueeze(-1))
                mask_tgt = (1-cls_prob_mask.float()) * -1000000.
                try:
                    ...
                    sim = (sim + mask_tgt)#'''
                output_dict["scores"] = 100*sim + image_info[:, :, -1:] 
                return output_dict #'''
            #'''avg 
            mask_text = 1-((input_ids==0)+(input_ids==101)+(input_ids==102)).float()
            avg_text = torch.sum(sequence_output_t*mask_text.unsqueeze(-1), 1) 
            avg_text = avg_text / torch.sum(mask_text, 1, keepdim=True)

            sim = torch.matmul(sequence_output_v, avg_text.unsqueeze(-1))
            output_dict["scores"] = 1000*sim + image_info[:, :, -1:] 
            return output_dict #'''
        elif self.training_head_type == 'flickr30k_retrieval' or self.training_head_type == 'coco_retrieval':
            batch_size = num_options[0]
...
class VisualBERT(BaseModel):
    def __init__(self, config):
        ...
        vocab_tot = torch.sum(self.vocab_map, -1, keepdim=True)
        vocab_tot[vocab_tot==0] = 1
        self.vocab_map = self.vocab_map / vocab_tot
    ...
    def update_sample_list_based_on_head(self, sample_list):
        ...
        elif self.config.training_head_type == 'flickr30k_retrieval' or self.config.training_head_type == 'coco_retrieval':
        ...
        multiple_choice_idx = getattr(image_info, "multiple_choice_idx", None)
        ...
        sample_list.multiple_choice_idx = multiple_choice_idx
        sample_list.image_mask = image_attention_mask
    ...
    def add_custom_params(self, sample_list):
        ...
        # Prepare Mask
        if sample_list.image_mask is None:
    ...
    def forward(self, sample_list):
        ...
        num_options = [sample_list.input_ids.size(0), sample_list.input_ids.size(1)]
        ...
        if getattr(sample_list, "cls_prob", None) is not None:
            #sample_list.image_text_label = sample_list.cls_prob
            image_text_label = torch.matmul(sample_list.cls_prob, self.vocab_map.to(sample_list.cls_prob.device))
        else:
            image_text_label = None
        ...
        output_dict = self.model(
            ...
            sample_list.masked_lm_labels,
            multiple_choice_idx=sample_list.multiple_choice_idx,
            num_options = num_options,
            cls_prob_mask=cls_prob_mask,
            image_text_label=image_text_label,  
            image_info=sample_list.image_info,
            start_ind = start_ind,
            end_ind = end_ind,
        )
