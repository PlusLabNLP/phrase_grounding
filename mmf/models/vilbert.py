...
class ViLBERTForClassification(nn.Module):
    def __init__(self, config):
        ...
        if self.config.training_head_type == 'visual7w' or self.config.training_head_type == 'flickr30k_retrieval':
    ...
    def forward(
        ...
        if self.training_head_type == 'flickr30k_retrieval':
            ...
            image_attention_mask = image_attention_mask.view(-1, image_attention_mask.size(2)) if image_attention_mask is not None else None
        ...
        if self.training_head_type == 'visual7w':
            ...
            act = nn.Sigmoid()
            logits = act(logits)
            output["scores"] = logits #logits[:, -self.config.num_multiple_choices:].squeeze()
        elif self.training_head_type == 'flickr30k_retrieval':
...
class ViLBERT(BaseModel):
    ...
    def get_image_and_text_features(self, sample_list):
        ...
        return {
            ...
            "multiple_choice_idx": multiple_choice_idx,
        }
    ...
    def forward(self, sample_list):
        ...
        output_dict = self.model(
            ...
            params["image_label"],
            params["image_target"],
            multiple_choice_idx=params["multiple_choice_idx"],
        )
