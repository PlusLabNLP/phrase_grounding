...
class ElectraVisioLinguisticEmbeddings(BertVisioLinguisticEmbeddings):
    def __init__(self, config, *args, **kwargs):
        super().__init__(config)
        ...
        self.bbox_projection = nn.Linear(5, config.embedding_size)
    ...
    def forward(
        ...
        if visual_embeddings is not None:
            visual_embeddings = self.projection(visual_embeddings)
            if not flowlayer is None:
                visual_embeddings = flowlayer(visual_embeddings)
            token_type_embeddings_visual = self.token_type_embeddings_visual(
                visual_embeddings_type
            )
            if image_info is not None:
                bbox_embeddings = self.bbox_projection(image_info)
            else:
                bbox_embeddings = torch.zeros_like(visual_embeddings)
...
class ImageBertVisioLinguisticEmbeddings(BertEmbeddings):
    def __init__(self, config, *args, **kwargs):
        super().__init__(config)
        self.token_type_embeddings_visual = nn.Embedding(
            config.type_vocab_size, config.hidden_size
        ...
        self.projection = nn.Linear(config.visual_embedding_dim, config.hidden_size)
        self.bbox_projection = nn.Linear(5, config.hidden_size)
    ...
    def forward(
        ...
        embeddings = self.dropout(embeddings)
        return embeddings, visual_embeddings
class TempImageBertVisioLinguisticEmbeddings(BertEmbeddings):
    def __init__(self, config, *args, **kwargs):
        super().__init__(config)
        self.token_type_embeddings_visual = nn.Embedding(
            config.type_vocab_size, config.hidden_size
