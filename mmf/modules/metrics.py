...
@registry.register_metric("v7w_accuracy")
class V7WAccuracy(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        """
        #output = model_output["scores"].contiguous().view(-1)
        #expected = sample_list["targets"].contiguous().view(-1)
        #output = (output > 0.5)
        #expected = (expected > 0.5)
        output = model_output["scores"] #.contiguous().view(-1)
        expected = sample_list["targets"] #.contiguous().view(-1)
        output = output.argmax(1)
        expected = expected.argmax(1)
        #output = (output > 0.5)
        #expected = (expected > 0.5)
...
@registry.register_metric("refcoco_accuracy")
class RefCOCOAccuracy(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def __init__(self):
        super().__init__("refcoco_accuracy")
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        expected = sample_list["targets"] #.contiguous().view(-1)
        output = output.argmax(1)
        expected = expected.squeeze(2).gather(1, output.view(-1, 1))
        correct = torch.sum(expected>0.00).float()
        total = len(expected)
